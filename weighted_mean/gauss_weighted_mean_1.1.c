...
struct header_struct {
  ...
  //---  Needed for threads
  double *DENSITY, *WEIGHT;  //--- Pointer to data arrays
...
int main(int argc, char **argv)
{
  ...
  if(header_info.Sampling != 1) {
    ...
    header_info.DENSITY = dvector(header_info.N_Data);
  }
  else{
    printf("Allocating %d for sampling density\n",header_info.N_Sampling);
    header_info.DENSITY = dvector(header_info.N_Sampling);
  }

  //------------------------------
  //--- Read weights from file
  //------------------------------
  if(header_info.Weights == 1) header_info.WEIGHT = read_weight(header_info.file_weights);
...
void *get_densities(void *Range)
{
  ...
  double sigma,sigma2, sigma_4;
  int    GridSize;
  char   Weights,WindowType;
  ...
  for(i=ini; i<fin; i++){
	...
	if (WindowType != 1){
	  ...
	  if(fabs(xp) > (sigma_4)) continue;
	  if(fabs(yp) > (sigma_4)) continue;
	  if(fabs(zp) > (sigma_4)) continue;
	
	  //--- Squared distance from particle
	  dista2 = xp*xp + yp*yp + zp*zp;

	  //--- Include weights
	  weight_temp = header_info.WEIGHT[neighbour_id];

	  double gauss_eval = exp( -(dista2/2.0) / sigma2 );
	  density_cumulative += gauss_eval * weight_temp;
	  denomin_cumulative += gauss_eval;
	}
	//----------------------------------
	//--- TopHat densities
	//----------------------------------
    ...
    header_info.DENSITY[i] = density_cumulative / denomin_cumulative;
