/*
   ...
   DESCRIPTION:
    Compute densities inside a gaussian window. It uses an indexed
       array to speed up the neighbour search. Particles are stored inside
       a grid so instead of searching through the entire box one limits the
   ...
   EXAMPLE:
    We want to compute top-hat densities inside a window of 3 Mpc. The
    input file comes from a 150 Mpc simulation (snap150Mpc). We can divide the box in
    a grid of 50x50x50 voxels.

      ./gauss_density_cgal snap150Mpc -g 50 -w 3000 snap150Mpc.den
    ...
    Compile as:
       gcc  -O3 gauss_density_cgal_all_threads.4.0.c -o gauss_density_cgal_all_threads-4 -lm -lpthread -D_REENTRANT
    ...
    History:
	   ...
	   - 13/Oct/2011: Add option for adaptive kernel
	   - 05/Nov/2011: Add normalization for unitary integral
	   - 24/May/2013: Fix bug in use of header_info.DENSITY.
*/
...
struct header_struct {
  ...
  char   Adaptive_ker;       //--- Use adaptive kernel from file
  //---  Files
  char   file_input   [256];
  char   file_output  [256];
  char   file_sampling[256];
  char   file_weights [256];
  char   file_kernel  [256];
  //---  Needed for threads
  double *DENSITY, *WEIGHT, *KERNEL;  //--- Pointer to data arrays
...
int main(int argc, char **argv)
{
  ...
  int       i,j,k;
  double    *density=NULL, *weight=NULL, *kernel=NULL;
  ...
  //------------------------------
  if(header_info.Sampling != 1) header_info.DENSITY = dvector(header_info.N_Data);
  else                          header_info.DENSITY = dvector(header_info.N_Sampling);
  ...
  //------------------------------
  //--- Read adaptive kernel from file
  //------------------------------
  if(header_info.Adaptive_ker == 1) kernel = read_weight(header_info.file_kernel);
  ...
  //header_info.DENSITY   = density;
  header_info.KERNEL    = kernel;
  ...
  printf("|=============================================================|\n");
  printf(" ");
  //--- Size of each chunck of data going to each thread
  i_med = header_info.N_Sampling/(header_info.N_Threads);
  //--- Loop over threads
  for(i=0; i < header_info.N_Threads; i++) {
    ...
    //--- Print some info
    printf("\n>>> Thread [%d] in range: [%d - %d] \n", i+1, Range[i].ini, Range[i].fin);

    
    //--- Call thread
    if ( (pthread_create( &thread_id[i], NULL, get_densities, (void *) &Range[i] )) ) {
  ...
  //------------------------------
  save_density(header_info.file_output, header_info.N_Sampling, header_info.DENSITY);
...
void *get_densities(void *Range)
{
  ...
  double sigma,sigma2;
  int    GridSize;
  char   Weights, WindowType, FLAG_ADAPTIVE_KER;
  double weight_temp = 1.0;                 //--- Default unitary weight
  double SQRT_2PI = pow(6.2831855,0.5);
  ...
  sigma      = header_info.WindowSize;
  GridSize   = header_info.GridSize;
  WindowType = header_info.WindowType; 
  Weights    = header_info.Weights;
  FLAG_ADAPTIVE_KER = header_info.Adaptive_ker;
  ...
  for(i=ini; i<fin; i++){
    ...
    //--- Use adaptive kernel
    if (FLAG_ADAPTIVE_KER == 1){
	sigma = header_info.KERNEL[i];
    } // end if (FLAG_ADAPTIVE_KER == 1)

    sigma2 = sigma*sigma;
    double n_dim = 3;
    double denom = pow(SQRT_2PI*sigma,n_dim);
	...
	if(fabs(xp) > (sigma*8.0)) continue;
	if(fabs(yp) > (sigma*8.0)) continue;
	if(fabs(zp) > (sigma*8.0)) continue;
	
	//--- Squared distance from particle
	dista2 = xp*xp + yp*yp + zp*zp;

	//--- Include weights if necessary
	if(Weights == 1) weight_temp = header_info.WEIGHT[neighbour_id];


	//----------------------------------
	//--- Gaussian or TopHat densities
	//----------------------------------
	if (WindowType != 1){
	  density_cumulative += exp(-(dista2/2.0)/(sigma2))*weight_temp/denom;
	}
	else{
	  if(dista2 <= sigma2)
	    density_cumulative += weight_temp;
	}
    ...
    header_info.DENSITY[i] = density_cumulative;
...
void read_command_line(int argc, char **argv, struct header_struct *header_info){
  ...
  for (i = 1; i < argc; i++) { 
    ...
    //--- KERNEL
      ...
      header_info->Adaptive_ker = 1;
      sscanf(argv[i+1],"%s",&header_info->file_kernel);
      printf(">>>     Adaptive kernel file: '%s'\n",header_info->file_kernel);
    }
    //--- NUMBER OF THREADS
    else if (strcmp( "-T", argv[i]) == 0){
...
struct particle_data *read_particles(char *fname, int *N_part, double *BoxSize)
{
  ...
  *BoxSize = (double) TempF;
  ...
  for(n=0; n< (*N_part); n++) {
    fread(&Part[n].Pos[0], sizeof(float), 1, fd);
    fread(&Part[n].Pos[1], sizeof(float), 1, fd);
    fread(&Part[n].Pos[2], sizeof(float), 1, fd);
  }
...
struct particle_data *read_particles_gadget(char *fname, int files, int *N_part, double *BoxSize)
{
  ...
  sprintf(buf,"%s",fname);
  printf("Attempting to open Gadget file[%d] %s\n...", files, buf);
